...
/**
 * ==================================================
 * 🛰️ 方糖情报雷达 - 运营增强版 (V2.6.0)
 * ==================================================
 * 1. 修复刷新后配置丢失问题（强持久化读取）
 * 2. 独立 API 存储与管理模块（支持修改/删除）
 * 3. 首页探测按钮智能常驻逻辑
 * 4. 移动端全响应式深度适配
 */
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const unsubConfig = onSnapshot(doc(db, 'artifacts', APP_ID_DB, 'public', 'data', 'config', 'main'), (snap) => {
      if (snap.exists()) {
        ...
        // 同步 API Key 和策略的变化 (如果其他终端修改了，这里也会更新)
  ...
  const openRouterFetch = async (prompt, systemPrompt, tempApiKey = null) => {
    ...
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        'X-Title': 'Sugar Radar Persistent'
      },
      body: JSON.stringify({
        model: model,
        messages: [
  ...
  const triggerUpdate = async (isAuto = false) => {
    ...
    try {
      const sys = "你是一个全球商业情报官。请抓取情报并严格返回 JSON 格式：{ 'items': [ { 'title', 'content', 'impact', 'suggestion', 'companies': [], 'target_audience', 'attention_worth', 'worth_reason' } ] }。不要 Markdown。";
      const userReq = `基于策略搜集 ${count} 条关于 ${strategy} 的最新情报动态。必须识别公司。`;
      
      const rawText = await openRouterFetch(userReq, sys);
      
      // 正则强化解析
      const jsonMatch = rawText.match(/[\{\[]([\s\S]*?)[\}\]]/);
      if (!jsonMatch) throw new Error("AI 响应未包含有效的 JSON 结构");
      const cleanJson = rawText.substring(rawText.indexOf(jsonMatch[0]), rawText.lastIndexOf(jsonMatch[0]) + jsonMatch[0].length);
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      setErrorMsg("✅ 配置已永久保存至云端");
      setTimeout(() => setErrorMsg(null), 3000);
    } catch (e) {
      setErrorMsg("保存失败，请检查网络权限");
    }
  };

  // API 彻底删除逻辑
  const handleDeleteKey = async () => {
    if (!confirm("确定要彻底销毁当前的 API 密钥吗？销毁后雷达将停止工作。")) return;
    try {
      await setDoc(doc(db, 'artifacts', APP_ID_DB, 'public', 'data', 'config', 'main'), {
        apiKey: deleteField()
      }, { merge: true });
  ...
  const handleTestConnection = async () => {
    ...
    try {
        await openRouterFetch("Ping test", "Test mode", apiKey);
        setTestStatus('success');
        setTimeout(() => setTestStatus(null), 3000);
    } catch (e) {
        setTestStatus('error');
        setErrorMsg("API 握手测试失败: " + e.message);
    }
  };
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-[#1E293B] font-sans selection:bg-indigo-100 antialiased overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full h-16 bg-white/80 backdrop-blur-xl border-b border-slate-100 flex items-center justify-between px-4 md:px-8 z-50">
        ...
        <button 
          ...
          className={`w-10 h-10 flex items-center justify-center rounded-full transition-colors ${view === 'home' ? 'bg-slate-50 text-slate-400 hover:text-indigo-600' : 'bg-indigo-600 text-white'}`}
        >
      ...
      <main className="pt-24 pb-20 px-4 md:px-8 max-w-5xl mx-auto">
        {errorMsg && (
          <div className={`mb-8 p-4 rounded-2xl flex items-start gap-3 text-sm font-bold animate-in fade-in slide-in-from-top-2 border ${errorMsg.includes('✅') ? 'bg-indigo-50 border-indigo-100 text-indigo-600' : 'bg-red-50 border-red-100 text-red-600'}`}>
            ...
            <span className="flex-1 break-all leading-tight uppercase">{errorMsg}</span>
          </div>
        )}
          ...
          <div className="animate-in fade-in duration-700">
            <header className="mb-10 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <h2 className="text-3xl md:text-4xl font-black tracking-tight text-slate-900 leading-none">实时情报动态</h2>
              </div>
              
              {/* 智能常驻按钮：如果没有 API，按钮变为功能引导 */}
              <button 
              ...
              >
                {isUpdating ? <RefreshCw size={18} className="animate-spin text-indigo-400" /> : <RefreshCw size={18} />}
                {!apiKey ? '请在后台配置 API 以开启功能' : manualCooldown > 0 ? `冷却中 (${manualCooldown}s)` : '探测最新 (5条)'}
              </button>
            </header>
            ...
            <div className="relative">
              ...
              <div className="space-y-6 md:space-y-12">
                {intelList.length === 0 && !isUpdating && <div className="pl-8 md:pl-24 py-24 text-slate-300 italic font-medium">情报雷达正在守望中，请确保已配置 API 密钥...</div>}
                {intelList.map((item) => {
                        ...
                        {item.companies && item.companies.length > 0 && (
                          <div className="flex flex-wrap gap-1.5 mb-5">
                            {item.companies.map((c, i) => (
                              <span key={i} className="bg-slate-900 text-white text-[10px] font-black px-2.5 py-1 rounded-lg uppercase flex items-center gap-1.5 shadow-sm"><Building2 size={10}/>{c}</span>
                            ))}
                          </div>
                        )}
        ...
        {view === 'admin' && !user && <div className="text-center py-20 text-slate-400 animate-pulse">正在激活加密链路...</div>}
          ...
          <div className="min-h-[60vh] flex items-center justify-center animate-in zoom-in-95">
            <div className="max-w-md w-full bg-white p-10 md:p-12 rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-100 shadow-2xl text-center">
              ...
              <input 
                ...
                placeholder="PIN" 
              />
              <button onClick={() => password === 'admin' ? setView('dashboard') : setErrorMsg("身份验证失败，请核对 PIN")} className="w-full h-16 bg-indigo-600 text-white rounded-2xl font-black text-lg active:scale-95 transition-all shadow-lg shadow-indigo-100">进入运营控制台</button>
            </div>
          </div>
        )}
              ...
              <div className="lg:col-span-8 space-y-10">
                ...
                <div className="bg-white p-8 md:p-10 rounded-[2.5rem] md:rounded-[3rem] border border-slate-100 shadow-sm relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center mb-8">
                        <h4 className="font-black text-xl flex items-center gap-2"><Key size={20}/> 引擎与永久配置</h4>
                        {testStatus === 'success' && <span className="flex items-center gap-1 text-[10px] font-black text-green-600 uppercase tracking-widest animate-bounce"><CheckCircle2 size={12}/> 通讯握手成功</span>}
                    </div>
                    ...
                    <div className="space-y-6">
                        <div>
                            <div className="flex justify-between items-end mb-1.5 px-1">
                                ...
                                <button onClick={handleDeleteKey} className="text-[9px] font-black text-red-400 hover:text-red-600 flex items-center gap-1 uppercase transition-colors"><Trash2 size={10}/> 彻底销毁密钥</button>
                            </div>
                            <input 
                                type="password" 
                                value={apiKey} 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block ml-1">执行模型 (MODEL ID)</label>
                                <input 
                                    type="text" 
                                    value={model} 
                            ...
                            <div className="flex items-end">
                                <button 
                                    ...
                                    className={`w-full h-14 rounded-2xl font-bold text-xs transition-all ${testStatus === 'testing' ? 'bg-slate-100 text-slate-400' : 'bg-slate-100 text-indigo-600 hover:bg-indigo-50 active:scale-95 shadow-sm'}`}
                                >
                                    {testStatus === 'testing' ? '正在连接 AI 引擎...' : '测试连通性'}
                                </button>
                            </div>
                        </div>
                    </div>
                    <p className="mt-4 text-[9px] text-slate-400 leading-relaxed font-medium uppercase tracking-tighter italic">※ 提示：修改 API Key 后必须点击下方的“保存同步配置”按钮，才能永久固化到云端，刷新后不再消失。</p>
                </div>
                ...
                <div className="bg-white p-8 md:p-10 rounded-[2.5rem] md:rounded-[3rem] border border-slate-100 shadow-sm">
                  ...
                  <textarea value={strategy} onChange={(e)=>setStrategy(e.target.value)} className="w-full h-64 p-8 bg-slate-50 border-none rounded-3xl text-lg font-bold outline-none mb-8 focus:ring-2 ring-indigo-100 transition-all custom-scrollbar" placeholder="输入你想关注的动态领域..." />
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <button onClick={handleSaveSettings} className="h-16 bg-slate-900 text-white rounded-2xl font-black active:scale-95 transition-all shadow-lg hover:bg-black">保存所有同步配置</button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100">
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase">当前耗材估算</p>
                    ...
                    <div className="bg-indigo-600 p-6 rounded-[2.5rem] shadow-lg shadow-indigo-100 text-white">
                        <p className="text-[10px] font-black text-indigo-200 uppercase tracking-widest">预估日运营成本</p>
                    ...
                    {logs.map(log => (
                      <div key={log.id} className="p-5 bg-slate-50 rounded-2xl border border-slate-100 transition-all hover:border-indigo-100">
                        <div className="flex justify-between mb-2 text-[10px] font-black uppercase">
                          <span className={log.type === 'AUTO_SLOT' ? 'text-indigo-600' : 'text-amber-600'}>{log.type === 'AUTO_SLOT' ? '自动轮巡' : '手动干预'}</span>
                          <span className="text-slate-400 font-mono">{log.createdAt ? new Date(log.createdAt.seconds * 1000).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}) : '...'}</span>
                        </div>
                        <div className="text-[13px] font-bold text-slate-700 leading-tight">同步了 {log.count} 条信号动态</div>
                      </div>
                    ))}
                    {logs.length === 0 && <p className="text-center py-20 text-slate-300 text-xs italic">暂无执行历史</p>}
                  </div>
                </div>
              </div>
            </div>
